...
async function updateWalletData() {
  ...
  try {
    ...
    addLog("Satukan Solidaritas Bantu Palestina!");
    addLog("https://digital.dompetdhuafa.org/donasi/jagapalestina");
    return walletData;
  } catch (error) {
...
const screen = blessed.screen({
  smartCSR: true,
  title: "XOS TESTNET AUTO BOT by SOUIY",
  autoPadding: true,
  fullUnicode: true,
  mouse: true,
...
function safeRender() {
  ...
  setTimeout(() => {
    try {
      if (!isHeaderRendered) {
        figlet.text("Airdrop Refferal", { font: "ANSI Shadow" }, (err, data) => {
...
function updateStatus() {
  try {
    ...
    const status = activityRunning
      ...
      : chalk.green("Idle");
    const statusText = `Status: ${status} | Active Account: ${getShortAddress(walletInfo.address)} | Total Accounts: ${accounts.length} | Auto Swap: ${dailyActivityConfig.swapRepetitions}x | XOS TESTNET AUTO BOT | FOLLOW TIKTOK : AirdropRefferal (Souiy1)`;
